...
- チームのプレースタイルを表す試合スタッツをチーム単位で平均し、似た特徴のチームを分類する。
- J1はシュート数・CK数・FK数・先制傾向を使用する。
- 欧州5大リーグはシュート数・支配率・パス成功率・枠内シュート率・決定機数・相手パス許容数・先制傾向を使用する。
- 利用できる特徴量が異なるため、J1と欧州は別々の特徴空間でクラスタリングする。
...
J1結果は通常の条件分析でも欧州分析JSONへ混在させず、クラスタリングでもJ1と欧州を別モデルとして扱う。
